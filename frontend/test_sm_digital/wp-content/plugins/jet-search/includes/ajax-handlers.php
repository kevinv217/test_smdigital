...
if ( ! class_exists( 'Jet_Search_Ajax_Handlers' ) ) {
	...
	class Jet_Search_Ajax_Handlers {
		...
		public function start_session() {
			...
			if ( false === $use_session ) {
				add_option( 'jet_search_suggestions_use_session' , "false" );
			}
		...
		*/
		...
		public function tax_terms_search_clauses( $args, $query ) {
			...
			$search_taxonomy = $_GET['data']['search_taxonomy'];
			$category__in    = $_GET['data']['category__in'];
			...
			$include_terms_ids = $_GET['data']['include_terms_ids'];
			$exclude_terms_ids = $_GET['data']['exclude_terms_ids'];
			$exclude_posts_ids = $_GET['data']['exclude_posts_ids'];
